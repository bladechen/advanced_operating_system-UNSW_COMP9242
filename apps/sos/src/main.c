...
static struct proc * test_process2;
...
void syscall_loop(seL4_CPtr ep)
{
	while (1) {
		...
		else if(label == seL4_VMFault)
		{
			...
			if (badge & TEMP_ONE_PROCESS_BADGE) {
				set_current_app_proc(test_process);
				dprintf(0, "process1111111111\n");
			} 

			if (badge & TEMP_TWO_PROCESS_BADGE) {
				set_current_app_proc(test_process2);
				dprintf(0, "process222222222\n");
			} 
